...
class WkHtmlToPdf
{
     ...
     */
    public function saveAs($filename, $ignore_error = false)
    {
        $pdfFile = $this->getPdfFilename();
        if ($ignore_error || is_null($this->error)) {
    ...
    {
        if ($this->tmpFile===null) {
            $this->tmpFile = tempnam($this->getTmpDir(),'tmp_WkHtmlToPdf_');
            $this->createPdf($this->tmpFile); //忽略错误
        }
        return $this->tmpFile;
    }
    ...
    protected function createPdf($fileName)
    {
        ...
        $process = proc_open($command, $descriptors, $pipes, null, 
                            $this->procEnv, array('bypass_shell'=>true));
